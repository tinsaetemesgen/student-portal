...
const ParentAnnouncements = () => {
    ...
    const parentAnnouncements = announcements.filter(
        (a) => a.status === "Published" && (a.audience === "All" || a.audience === "Parents")
    );
        ...
        <DashboardLayout role="parent">
            <div className="space-y-6">
                <div>
                    ...
                    <p className="text-gray-500">View announcements relevant to parents</p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="bg-white rounded-xl shadow-sm p-5 flex items-center gap-4">
                        <div className="bg-blue-100 p-3 rounded-lg text-blue-600"><Megaphone size={25} /></div>
                        <div><p className="text-gray-500 text-sm">Total</p><h2 className="text-2xl font-bold">{parentAnnouncements.length}</h2></div>
                    </div>
                    <div className="bg-white rounded-xl shadow-sm p-5 flex items-center gap-4">
                        <div className="bg-green-100 p-3 rounded-lg text-green-600"><CheckCircle size={25} /></div>
                        <div><p className="text-gray-500 text-sm">For Parents</p><h2 className="text-2xl font-bold">{parentAnnouncements.filter(a => a.audience === "Parents" || a.audience === "All").length}</h2></div>
                    </div>
                    <div className="bg-white rounded-xl shadow-sm p-5 flex items-center gap-4">
